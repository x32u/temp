...
async def blacklist(ctx: commands.Context):
    ...
    embed = discord.Embed(
        color=Colors.color,
        description="Do you **agree** to our [privacy policy](https://evict.cc/privacy) and for your data to be used for commands?\n**DISAGREEING** will result in a blacklist from using bot's commands",
    )
    yes = discord.ui.Button(emoji=Emojis.approve, style=discord.ButtonStyle.gray)
    no = discord.ui.Button(emoji=Emojis.deny, style=discord.ButtonStyle.gray)
    ...
    async def yes_callback(interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="Evict Logs",
            description="User agreed to Evict's privacy policy & terms of service.",
            color=Colors.color,
        )
    ...
    async def no_callback(interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            embed=discord.Embed(
                color=Colors.color,
                description=f"You got blacklisted from using bot's commands. If this is a mistake, please check our [**support server**](https://discord.gg/evict)",
            ),
            view=None,
        )
        ...
        embed = discord.Embed(
            title="Evict Logs",
            description="User got blacklisted for saying no on callback.",
            color=Colors.color,
        )
        embed.add_field(name="User", value=f"{interaction.user}", inline=False)
        embed.add_field(name="User ID", value=f"{interaction.user.id}", inline=False)
...
async def restricted_command(ctx: commands.Context):
    ...
    if check := await ctx.bot.db.fetch(
        "SELECT * FROM restrictcommand WHERE guild_id = $1 AND command = $2",
        ctx.guild.id,
        ...
        for row in check:
            ...
            if not role:
                await ctx.bot.db.execute(
                    "DELETE FROM restrictcommand WHERE role_id = $1", row["role_id"]
                )
