...
class donor(commands.Cog):
    ...
    async def hardunban(self, ctx: EvictContext, *, member: discord.User):
        ...
        if che is None:
            return await ctx.warning(f"{member} is **not** hardbanned.")
    ...
    @commands.command(
        description="hardban a user from the server",
        usage="[user]",
        brief="donor & administrator",
    )
    @Permissions.has_permission(administrator=True)
    ...
    @commands.command(
        aliases=["hardbanlist"],
        description="check hardban list",
        brief="donor & administrator",
    )
    async def hardbanned(self, ctx: EvictContext):

        results = await self.bot.db.fetch(
            "SELECT * FROM hardban WHERE guild_id = $1", ctx.guild.id
        )

        if len(results) == 0:
            return await ctx.warning("No **hardbanned** members found.")

        hardban_list = [
            f"``{index + 1}.`` {await self.bot.fetch_user(result['banned'])} (``{result['banned']}``)"
    ...
    async def uwulock(self, ctx: EvictContext, *, member: discord.Member):
        ...
        if member.id == ctx.bot.user.id:
            return await ctx.warning("I **cannot** uwulock myself.")
        ...
        if check1 is not None:
            return await ctx.warning(
                f"**{member}** is already globaluwulocked, this must be removed by bot owner."
            )
        if check is None:
            await self.bot.db.execute(
    ...
    @commands.command(
        aliases=["uwulocklist"],
        description="check uwulock list",
        brief="donor & manage messages",
    )
    async def uwulocked(self, ctx: EvictContext):

        results = await self.bot.db.fetch(
            "SELECT * FROM uwulock WHERE guild_id = $1", ctx.guild.id
        )
    ...
    @commands.command(
        description="remove everyone from uwulock", brief="donor & server owner"
    )
    async def uwulockreset(self, ctx: EvictContext):

        check = await self.bot.db.fetchrow(
            "SELECT guild_id FROM uwulock WHERE guild_id = {}".format(ctx.guild.id)
        )
        if check is None:
            return await ctx.warning("There is **no one** in uwulock.")

        else:
            await self.bot.db.execute(
                "DELETE FROM uwulock WHERE guild_id = {}".format(ctx.guild.id)
            )
        await ctx.success("I have **successfully** removed everyone from uwulock.")

    @Permissions.server_owner()
    @commands.command(
        description="remove everyone from hardban", brief="donor & server owner"
    )
    async def hardbanreset(self, ctx: EvictContext):

        check = await self.bot.db.fetchrow(
            "SELECT guild_id FROM hardban WHERE guild_id = {}".format(ctx.guild.id)
        )
        if check is None:
            return await ctx.warning("There is **no one** in hardban.")

        else:
            await self.bot.db.execute(
                "DELETE FROM hardban WHERE guild_id = {}".format(ctx.guild.id)
            )
    ...
    async def shutup(self, ctx: EvictContext, *, member: discord.Member):
        ...
        if member.id == ctx.bot.user.id:
            return await ctx.warning("Do not shutup me.")
    ...
    @commands.command(
        aliases=["stfureset"],
        description="remove everyone from uwulock",
        brief="donor & server owner",
    )
    async def shutupreset(self, ctx: EvictContext):

        check = await self.bot.db.fetchrow(
            "SELECT guild_id FROM shutup WHERE guild_id = {}".format(ctx.guild.id)
        )
        if check is None:
            return await ctx.warning("There is **no one** in uwulock.")
            ...
            await self.bot.db.execute(
                "DELETE FROM shutup WHERE guild_id = {}".format(ctx.guild.id)
            )
        await ctx.success("I have **successfully** removed everyone from uwulock.")
    ...
    @commands.command(
        description="force nickname a user",
    ...
    async def forcenick(
        self, ctx: EvictContext, member: discord.Member, *, nick: str = None
    ):
        ...
        if nick is None:
            ...
            if check is None:
                return await ctx.warning(f"**No** forcenick found for {member}.")
            ...
            await ctx.success(
                f"I have **successfully** removed the forcenick for {member.mention}."
            )
            ...
            await ctx.success(
                f"**successfully** added the forcenick for {member.mention}"
            )

    @commands.cooldown(1, 60, commands.BucketType.guild)
    @commands.command(
        aliases=["fnclear"],
        description="clear everyone's forcenickname",
        brief="donor & administrator",
    )
    @Permissions.has_permission(administrator=True)
    async def forcenickclear(self, ctx: EvictContext):
        ...
        await self.bot.db.execute(
            "DELETE FROM forcenick WHERE guild_id = {}".format(ctx.guild.id)
        )
    ...
    @commands.group(
        ...
        name="reskin",
        description="customize evicts output embeds",
    )
    ...
    @reskin.command(
        name="disable",
        description="disable the customization output messages",
        aliases=["off"],
        brief="donor",
    )
    ...
    @reskin.command(
        name="name",
        description="change the name used on evicts output embeds",
        brief="donor",
    )
    async def reskin_name(self, ctx: EvictContext, *, name: str = None):
        ...
        if name == None or name.lower() == "none":
            return await ctx.success(
                f"I have set your **reskin** name to `{ctx.author.name}`."
            )
    ...
    @reskin.command(
        name="avatar",
        description="change the icon used on evicts output embeds",
        aliases=["av"],
        brief="donor",
    )
    async def reskin_avatar(self, ctx: EvictContext, url: str = None):
        ...
        return await ctx.success(
            f"I have set your **reskin** avatar to [**image**]({url})."
        )
    ...
    async def impersonate(self, ctx: EvictContext, member: discord.Member, *, content):
        ...
        if member.id in self.bot.owner_ids:
            return await ctx.warning("you **cannot** mock a bot owner.")
        if member.id == ctx.bot.user.id:
            return await ctx.warning("you **cannot** mock me.")
        if member.id == ctx.guild.owner.id:
            return await ctx.warning("you **cannot** mock the server owner.")

        try:
            webhook = await ctx.channel.create_webhook(
                name=member.display_name, reason=f"{ctx.author} used mock command"
            )
