...
async def translate(interaction: discord.Interaction, message: discord.Message):
    if not message.content:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=Colors.color,
                description="{} {}: there is no message to translate".format(
                    interaction.client.warning, interaction.user.mention
                ),
            ),
            ephemeral=True,
        )
    ...
    async def select_callback(inter: discord.Interaction):
        if inter.user.id != interaction.user.id:
            return await inter.response.send_message(
                embed=discord.Embed(
                    color=Colors.color,
                    description="{} {}: You are not the author of this embed".format(
                        interaction.client.warning, inter.user.mention
                    ),
                ),
                ephemeral=True,
            )
        translated = GoogleTranslator(source="auto", target=select.values[0]).translate(
            message.content
        )
        e = discord.Embed(
            color=Colors.color,
            title="translated to {}".format(select.values[0]),
...
async def steal_emoji(interaction: discord.Interaction, message: discord.Message):
    if not interaction.user.guild_permissions.manage_emojis_and_stickers:
        return await interaction.response.send_message(
            embed=discord.Embed(
                description=f"{interaction.client.warning} {interaction.user.mention}: you are missing permission `manage_emojis_and_stickers`"
            ),
            ephemeral=True,
        )
    try:
            ...
            for emoji in interaction.guild.emojis:
                if added_emojis:
                    return await interaction.edit_original_response(
                        content="added emojis"
                    )
                try:
                    async with session.get(emoji.url) as resp:
                        image = io.BytesIO(await resp.read()).read()
                    added = await interaction.guild.create_custom_emoji(
                        name=emoji.name, image=image
                    )
                except Exception as error:
                    embed = discord.Embed(
                        color=Colors.color,
                        description=f"{Emojis.deny} {interaction.user.mention}: unable to add emojis - {error}",
                    )
...
async def steal_sticker(interaction: discord.Interaction, message: discord.Message):
    if not interaction.user.guild_permissions.manage_emojis_and_stickers:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=Colors.color,
                description=f"{interaction.client.warning} {interaction.user.mention}: you are missing permission `manage_expressions`",
            ),
            ephemeral=True,
        )
    if not message.stickers:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=Colors.color,
                description=f"{Emojis.deny} {interaction.user.mention}: this message doesnt have any sticker",
            ),
            ephemeral=True,
        )
    try:
        ...
        embed = discord.Embed(
            color=Colors.color,
            description=f"{Emojis.deny} {interaction.user.mention}: unable to add this sticker - {error}",
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def grab_sticker(interaction: discord.Interaction, message: discord.Message):
    if not message.stickers:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=Colors.color,
                description=f"{Emojis.deny} {interaction.user.mention}: this message doesn't have any sticker",
            ),
            ephemeral=True,
        )
    options = []
    for g in interaction.user.mutual_guilds:
        if (
            g.get_member(
                interaction.user.id
            ).guild_permissions.manage_emojis_and_stickers
            and len(g.stickers) < g.sticker_limit
        ):
            options.append(
                discord.SelectOption(
                    label=g.name, value=str(g.id), description=g.description
                )
            )
    ...
    if len(options) == 0:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=Colors.color,
                description=f"{Emojis.deny} {interaction.user.mention}: you cant add this sticker anywhere",
            ),
            ephemeral=True,
        )
    embed = discord.Embed(
        color=Colors.color,
        description=f"🔍 where should `{message.stickers[0].name}` be added?",
    )
    select = discord.ui.Select(
        options=options, placeholder="select a server", max_values=len(options)
    )
    ...
    async def select_callback(inte: discord.Interaction):
        ...
        for value in select.values:
            ...
            sticker = await inte.client.get_guild(int(value)).create_sticker(
                name=name,
                description=name,
                emoji="skull",
                file=file,
        ...
        await inte.response.edit_message(
            embed=discord.Embed(
                color=Colors.color,
                description=f"{Emojis.approve} {inte.user.mention}: added `{sticker.name}` in **{len(select.values)}** servers",
            ),
            view=None,
        )
...
class confessModal(Modal, title="confess here"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if check:
                ...
                if any(link in self.children[0].value for link in links):
                    return await interaction.client.ext.warning(
                        interaction,
                        "i can't send links. those things can be dangerous !!".capitalize(),
                        ephemeral=True,
                    )
                embed = discord.Embed(
                    color=Colors.color,
            ...
            return await interaction.client.ext.error(
                interaction, f"Couldn't send your confession - {e}"
            )
...
class Cog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        try:
            ...
            self.bot.tree.remove_command(
                "steal sticker", type=discord.AppCommandType.message
            )
            self.bot.tree.remove_command(
                "grab sticker", type=discord.AppCommandType.message
            )
            self.bot.tree.remove_command(
                "steal emoji", type=discord.AppCommandType.message
            )
            self.bot.tree.add_command(steal_sticker)
            self.bot.tree.add_command(grab_sticker)
            self.bot.tree.add_command(translate)
    ...
    @app_commands.command(
        name="confess", description="anonymously confess your thoughts"
    )
    async def confess(self, ctx: discord.Interaction):
        ...
        return await self.bot.ext.error(
            ctx, "confessions aren't enabled in this server", ephemeral=True
        )
