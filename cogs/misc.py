...
def is_detention():
    async def predicate(ctx: EvictContext):
        ...
        if not check:
            await ctx.warning("Naughty corner is not configured")
...
class misc(commands.Cog):
    ...
    async def nc_remove(self, ctx: EvictContext, *, member: discord.Member):
        ...
        if await Mod.check_hieracy(ctx, member):
            ...
            if not check:
                return await ctx.warning("this member is **not** in the naughty corner")
    ...
    async def webhook_name(self, ctx: EvictContext, code: str, *, name: str):
        ...
        if not check:
            return ctx.error("No **webhook** associated with this code")
        ...
        if webhook:
            ...
            return ctx.error(f"No **webhook** found")
    ...
    async def webhook_avatar(self, ctx: EvictContext, code: str, link: str = None):
        ...
        if not check:
            return ctx.error("There is no **webhook** associated with this code.")
    ...
    async def webhook_create(
        self, ctx: EvictContext, channel: discord.TextChannel, *, name: str = "evict"
    ):

        webhook = await channel.create_webhook(
            name=name, reason=f"webhook created by {ctx.author}"
        )
        code = "".join(
    ...
    @commands.command(
        description="make a channel nsfw for 30 seconds",
