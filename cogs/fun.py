...
class fun(commands.Cog):
    ...
    @commands.command(
        ...
        usage="[choices separated by a command]",
    )
    ...
    async def poll(self, ctx: EvictContext, *, question: str):
        message = await ctx.send(
            embed=discord.Embed(color=Colors.color, description=question).set_author(
        ...
        if member.id in self.bot.owner_ids:
            await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="how retarded",
                    description=f"{member.mention} is `0%` retarded <a:dumbass:1265487107196190802>",
                )
            )
        else:
            await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="how retarded",
                    description=f"{member.mention} is `{randrange(100)}%` retarded <a:dumbass:1265487107196190802>",
                )
            )
        ...
        if member.id in self.bot.owner_ids:
            return await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="gay r8",
                    description=f"{member.mention} is `0%` gay 🏳️‍🌈",
                )
            )
        else:
            await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="gay r8",
                    description=f"{member.mention} is `{randrange(100)}%` gay 🏳️‍🌈",
                )
            )
        ...
        if member.id in self.bot.owner_ids:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="cool r8",
                    description=f"{member.mention} is `{randrange(100)}%` cool 😎",
                )
            )
        ...
        if member.id in self.bot.owner_ids:
            return await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="iq test",
                    description=f"{member.mention} has `3000` iq 🧠",
                )
            )
        else:
            await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    title="iq test",
                    description=f"{member.mention} has `{randrange(100)}` iq 🧠",
                )
            )
    ...
    async def bitches(
        ...
        embed = discord.Embed(
            color=Colors.color, description=f"{user.mention} has `{result}` bitches"
        )
        if user.id in self.bot.owner_ids:
            embed = discord.Embed(
                color=Colors.color,
                description=f"{user.mention} has `1000000000000` bitches",
            )
        await ctx.reply(embed=embed)
    ...
    async def bird(self, ctx):
        ...
        await ctx.reply(
            file=discord.File(
                fp=BytesIO(await self.bot.session.read(data["file"])),
                filename="bird.png",
            )
        )
    ...
    async def dog(self, ctx):
        ...
        await ctx.reply(
            file=discord.File(
                fp=BytesIO(await self.bot.session.read(data["url"])),
    ...
    async def cat(self, ctx):
        ...
        await ctx.reply(
            file=discord.File(
                fp=BytesIO(await self.bot.session.read(data["url"])), filename="cat.png"
            )
        )
    ...
    async def capybara(self, ctx):
        ...
        await ctx.reply(
            file=discord.File(
                fp=BytesIO(await self.bot.session.read(data["data"]["url"])),
                filename="cat.png",
            )
        )
    ...
    async def snapchat(self, ctx: EvictContext, *, username: str):
        try:
            ...
            return await ctx.warning(f"{username} **does not** appear to be valid.")

    @commands.command(
        description="get a random TikTok video", aliases=["foryou", "foryoupage"]
    )
    async def fyp(self, ctx: EvictContext):

        async with ctx.typing():
            recommended = await self.session.get_json(
            ...
            final = await self.session.get_json(
                "https://tikwm.com/api/",
                params={
            ...
            await ctx.reply(
                embed=embed,
                file=discord.File(
                    fp=await self.session.getbyte(url=final["data"]["play"]),
    ...
    async def roblox(self, ctx: EvictContext, profile: str):
        try:
            ...
            embed.set_footer(
                text="Roblox", icon_url="https://cdn.resent.dev/roblox.jpg"
            )
            await ctx.reply(embed=embed)
        except Exception:
    ...
    async def snapchatuser(self, ctx: EvictContext, profile: str):
        try:
            ...
            embed.set_footer(
                text="Snapchat", icon_url="https://cdn.resent.dev/snapchat.jpg"
            )
            await ctx.reply(embed=embed)
        except Exception:
            return await ctx.warning(f"{profile} **does not** appear to be valid.")
    ...
    async def tiktok(self, ctx: EvictContext, profile: str):
        try:
            ...
            if data.verified:
                embed.add_field(name="verified:", value=f"{data.verified}", inline=True)
            if data.private:
                embed.add_field(name="private:", value=f"{data.private}")
            embed.set_thumbnail(url=data.avatar)
            embed.set_footer(
                text="TikTok", icon_url="https://cdn.resent.dev/tiktok.png"
            )
            await ctx.reply(embed=embed)
        except Exception:
            return await ctx.warning(f"{profile} **does not** appear to be valid.")
    ...
    async def instagram(self, ctx: EvictContext, profile: str):
        try:
            ...
            embed.set_footer(
                text="Instagram", icon_url="https://cdn.resent.dev/instagram.png"
            )
            await ctx.reply(embed=embed)
        except Exception:
            return await ctx.warning(f"{profile} **does not** appear to be valid.")
    ...
    async def pack(self, ctx: EvictContext, *, member: discord.Member):
        if member == ctx.author:
            return await ctx.warning(
                "You **cannot** pack yourself. I don't know why you would want to either."
            )
    ...
    async def tictactoe(self, ctx: EvictContext, *, member: discord.Member):
        if member is ctx.author:
            return await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{Emojis.warn} {ctx.author.mention}: You can't play with yourself. It's ridiculous",
                )
            )
        if member.bot:
        ...
        vi.message = await ctx.send(
            content=f"{member.mention}\n**{member.name}** vs **{ctx.author.name}**\n\nTic Tac Toe: **{ctx.author.name}** Is First",
            embed=None,
            view=vi,
    ...
    async def blacktea(self, ctx: EvictContext):
        ...
        for leader in leaderboard[::-1]:
            ...
            le += 1
        e = discord.Embed(
            color=Colors.color, title=f"leaderboard for blacktea", description=auto
        ).set_footer(
    ...
    async def search(self, ctx: EvictContext, *, query: str):
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://vile.bot/api/browser/search", data=query
            ) as response:
        ...
        for page, chunk in enumerate(chunks):
            embed = (
                discord.Embed(color=Colors.color, title="Search Results")
                .set_author(
                    name=ctx.author.name, icon_url=ctx.author.display_avatar.url
                )
                .set_footer(
                    text=f"Page {page+1}/{len(chunks)} Of Google Pages",
                    icon_url="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c1/Google_%22G%22_logo.svg/1200px-Google_%22G%22_logo.svg.png",
                )
            )
            for res in chunk:
                embed.add_field(
                    name=f"Google",
                    value=f"[**{res.get('title')}**](https://{res.get('domain')})\n{(res.get('description'))}",
                    inline=False,
                )
            embeds.append(embed)
