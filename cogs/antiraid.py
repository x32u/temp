...
class antiraid(commands.Cog):
    ...
    async def antiraid_settings(self, ctx: EvictContext):
        ...
        settings_enabled = {
            "massjoin": Emojis.deny,
            "defaultavatar": Emojis.deny,
            "newaccounts": Emojis.deny,
        }
        ...
        for result in results:
            if settings_enabled.get(result[0]):
                settings_enabled[result[0]] = Emojis.approve
    ...
    async def massjoin_enable(self, ctx: EvictContext, punishment: str, joins: int):
        ...
        if check:
            return await ctx.warning("Massjoin protection is **already** enabled")
    ...
    async def massjoin_disable(self, ctx: EvictContext):
        ...
        if not check:
            return await ctx.warning("Massjoin protection is **not** enabled")
    ...
    async def newaccounts_on(self, ctx: EvictContext, time: str, punishment: str):
        try:
            if not punishment in ["kick", "ban"]:
                return await ctx.error("Punishment is not either **kick** or **ban**")
            ...
            if check:
                return await ctx.warning("Newaccounts is **already** enabled")
            ...
            return await ctx.warning(f"**{time}** couldn't be converted in **seconds**")
    ...
    async def newaccounts_off(self, ctx: EvictContext):
        ...
        if check is None:
            return await ctx.warning("Newaccounts is **not** enabled")
    ...
    async def defaultpfp_on(self, ctx: EvictContext, punishment: str):
        ...
        if not punishment in ["kick", "ban"]:
            return await ctx.warning("Punishment can be either **ban** or **kick**")
        check = await self.bot.db.fetchrow(
            "SELECT * FROM antiraid WHERE guild_id = $1 AND command = $2",
            ctx.guild.id,
            "defaultavatar",
        ...
        if check:
            return await ctx.warning("Defaultavatar is **already** enabled")
        await self.bot.db.execute(
            "INSERT INTO antiraid VALUES ($1,$2,$3,$4)",
            ctx.guild.id,
            "defaultavatar",
    ...
    async def defaultpfp_off(self, ctx: EvictContext):
        ...
        if not check:
            return await ctx.warning("Defaultavatar is **not** enabled")
