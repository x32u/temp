...
async def lf_add_reactions(
    ctx: EvictContext, message: typing.Union[discord.Message, None]
):
    if message is None:
        return
    ...
    for r in reactions:
        try:
            ...
            return await ctx.send(
                "I cannot access your custom Last.FM reactions, please run ``;lf reactions`` to delete them."
            )
...
class lastfm(commands.Cog):
    ...
    async def lf_remove(self, ctx: EvictContext):
        ...
        await self.bot.db.execute(
            "DELETE FROM lastfm_users WHERE discord_user_id = {}".format(ctx.author.id)
        )
        await ctx.lastfm_message(
            "I have removed your **last.fm** account from the bot."
        )
    ...
    @lastfm.group(
        ...
        name="embed",
        description="create your own lastfm custom embed",
        aliases=["mode"],
    )
    async def lf_embed(self, ctx: EvictContext):
        await ctx.create_pages()

    @lf_embed.command(
        name="steal",
        description="steal someone's custom lastfm embed",
        usage="[member]",
    )
    async def lf_embed_steal(self, ctx: EvictContext, *, member: discord.Member):
        ...
        return await ctx.lastfm_message(
            f"I have successfully copied **{member.name}'s** custom lastfm embed."
        )

    @lf_embed.command(
        name="set",
        description="set a personal embed as the lastfm embed",
        usage="[message | embed code]",
    )
    async def lf_embed_set(self, ctx: EvictContext, *, embed: str):
    ...
    async def lf_embed_view(self, ctx: EvictContext):

        check = await self.bot.db.fetchrow(
            "SELECT * FROM lfmode WHERE user_id = $1", ctx.author.id
        )
        if not check:
            return await ctx.lastfm_message("You do not have a **last.fm** embed.")

        embed = discord.Embed(color=Colors.color, description=f"```{check['mode']}```")
        return await ctx.reply(embed=embed)

    @lf_embed.command(
        name="none", description="clear your last.fm custom embed", aliases=["delete"]
    )
    async def lf_embed_none(self, ctx: EvictContext):

        check = await self.bot.db.fetchrow(
            "SELECT * FROM lfmode WHERE user_id = $1", ctx.author.id
        )
        if not check:
            return await ctx.lastfm_message("You do not have any **last.fm** embed.")
    ...
    @lastfm.command(
        name="customcommand",
        description="set a custom command for nowplaying",
        usage="[command]",
        aliases=["cc"],
    )
    async def lf_customcommand(self, ctx: EvictContext, *, cmd: str):
        ...
        if cmd == "none":
            ...
            if check is None:
                return await ctx.lastfm_message(
                    f"You don't have a **last.fm** custom command"
                )
            ...
            return await ctx.lastfm_message(
                "Your **Last.fm** custom command got succesfully deleted."
            )
    ...
    @lastfm.command(
        name="topartists",
        aliases=["ta", "tar"],
        description="check a member's top 10 artists",
        usage="<member>",
    )
    async def lf_topartists(self, ctx: EvictContext, member: discord.Member = None):
    ...
    @lastfm.command(
        name="toptracks",
        aliases=["tt"],
        description="check a member's top 10 tracks",
        usage="<member>",
    )
    async def lf_toptracks(self, ctx: EvictContext, *, member: discord.Member = None):
    ...
    async def lf_login(self, ctx: EvictContext):
        """
        ...
        if check is not None:
            return await ctx.warning(
                "You **already** have a **Last.fm** account linked."
            )
    ...
    @lastfm.command(
        name="topalbums",
        aliases=["tal"],
        description="check a member's top 10 albums",
        usage="<member>",
    )
    async def lf_topalbums(self, ctx: EvictContext, *, member: discord.Member = None):
    ...
    async def lf_user(
        ...
        check = await self.bot.db.fetchrow(
            "SELECT username FROM lastfm_users WHERE discord_user_id = $1", user.id
        )
        username = check["username"]

        if not check:
            return await ctx.warning(
                f"{'You don' if user == ctx.author else f'**{user}** doesn'}'t have a **last.fm** account linked."
            )
    ...
    @lastfm.command(
        name="whoknows",
        aliases=["wk"],
        description="see who knows a certain artist in the server",
        usage="[artist]",
    )
    async def lf_whoknows(self, ctx: EvictContext, *, artist: str = None):
        ...
        for x in sorted_users:
            if x[1] != 0:
                num += 1
                rows.append(
                    f"{'<a:developer:1208257462549880913>' if num == 1 else f'`{num}`'} [**{x[0]}**]({x[2]}) has **{x[1]}** plays"
                )
        ...
        embed = discord.Embed(
            color=Colors.color,
            description="\n".join(rows),
            title=f"Top Listeners for {artist} in {ctx.guild.name}",
        )
        embed.set_author(
    ...
    @lastfm.command(
        name="globalwhoknows",
        aliases=["gwk"],
        description="see who knows a certain artist across all servers the bot is in",
        usage="[artist]",
    )
    ...
    @lastfm.command(
        name="cover",
        description="get the cover image of your lastfm song",
        usage="<member>",
    )
    async def lf_cover(
        ...
        check = await self.bot.db.fetchrow(
            "SELECT * FROM lastfm_users WHERE discord_user_id = {}".format(member.id)
        )
        if check is None:
            return await ctx.lastfm_message(
                "You don't have a **last.fm** account connected."
    ...
    @lastfm.command(
        name="reactions",
        description="add custom reactions to your lastfm embed",
        usage="[emojis | none]\nnone -> no reactions for np command\nno emoji -> default emojis will be used",
    )
    async def lf_reactions(self, ctx: EvictContext, *emojis: str):
        ...
        if len(emojis) == 0:
            if not check:
                return await ctx.lastfm_message(
                    "You don't have any **last.fm** custom reaction to remove."
                )
            ...
            return await ctx.lastfm_message(
                "I have **deleted** your **last.fm** custom reactions."
            )
        ...
        return await ctx.lastfm_message(
            f"Your **last.fm** reactions are {''.join([e for e in emojis])}"
        )

    @lastfm.command(
        name="howto", description="tutorial for using lastfm", aliases=["tutorial"]
    )
    async def lf_howto(self, ctx: EvictContext):
        await ctx.lastfm_message(
            "instructions can be viewed at <https://new-docs.evict.cc/configuration/lastfm>"
        )

    @lastfm.command(
        name="crowns", description="get the crowns of a member", usage="<user>"
    )
    async def lf_crowns(self, ctx: EvictContext, *, member: discord.User = None):
        ...
        if len(check) == 0:
            return await ctx.lastfm_message(
                "I **couldn't** find any crown for **{}**".format(member)
            )
        await ctx.typing()
    ...
    async def lf_chart(
        ...
        if user is None:
            user = ctx.author
        username = await self.bot.db.fetchrow(
            "SELECT * FROM lastfm_users WHERE discord_user_id = {}".format(user.id)
        )

        if not username:
            return await ctx.lastfm_send(
                f"{user.mention} does not have a lastfm account linked."
            )
        ...
        return await ctx.reply(
            f"Last.FM chart for {user.mention} **{size}** album chart ({period})",
    ...
    async def globalwhoknows(self, ctx: EvictContext, *, artist: str = None):
    ...
    async def whoknows(self, ctx: EvictContext, *, artist: str = None):
        await ctx.invoke(self.bot.get_command("lastfm whoknows"), artist=artist)

    @commands.command(
        aliases=["tal"], description="check a member's top 10 albums", usage="<member>"
    )
    async def topalbums(self, ctx: EvictContext, *, member: discord.Member = None):
        await ctx.invoke(self.bot.get_command("lastfm topalbums"), member=member)

    @commands.command(
        aliases=["tt"], description="check a member's top 10 tracks", usage="<member>"
    )
    async def toptracks(self, ctx: EvictContext, *, member: discord.Member = None):
    ...
    async def topartists(self, ctx: EvictContext, *, member: discord.Member = None):
    ...
    async def nowplaying(self, ctx: EvictContext, *, member: discord.User = None):
        ...
        if check:
            ...
            if starData is None:
                ...
                if user != "error":
                    ...
                    embed.set_footer(
                        ...
                        icon_url=(a["recenttracks"]["track"][0])["image"][3]["#text"],
                    )

                    view = discord.ui.View()
                    view.add_item(
                        discord.ui.Button(
                            label="Track Link", url=a["recenttracks"]["track"][0]["url"]
                        )
                    )
                    view.add_item(
                        discord.ui.Button(
                            label="Artist Link", url=f"https://last.fm/music/{artist}"
                        )
                    )
            ...
            return await ctx.lastfm_message(
                f"**{member}** doesn't have a **Last.fm account** linked. Use `{ctx.clean_prefix}lf login` to link your **account**."
            )
