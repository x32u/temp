...
class reactionroles(commands.Cog):
    ...
    async def rr_add(
        ...
        *,
        ...
        if check:
            return await ctx.warning("A similar reactionrole was already added")
        ...
        try:
            ...
            return await ctx.error("Unable to add reaction role for this role")
    ...
    async def rr_removeall(
        ...
        if len(results) == 0:
            return await ctx.warning("No **reactionroles** found")
        if channel:
    ...
    @reactionrole.command(
        name="list", description="list all the reaction roles from the server"
    )
