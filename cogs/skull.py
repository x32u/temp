...
class skull(commands.Cog):
    ...
    @skull.command(
        description="modify the skullboard count",
        brief="manage guild",
        usage="[count]",
        aliases=["amount"],
    )
    ...
    async def count(self, ctx: EvictContext, count: int):
        if count < 1:
            return await ctx.warning("Count can't be **less** than 1")
        check = await self.bot.db.fetchrow(
            "SELECT * FROM skullboard WHERE guild_id = $1", ctx.guild.id
        )
        if check is None:
            await self.bot.db.execute(
                "INSERT INTO skullboard (guild_id, count) VALUES ($1, $2)",
                ctx.guild.id,
                count,
            )
        else:
            await self.bot.db.execute(
                "UPDATE skullboard SET count = $1 WHERE guild_id = $2",
                count,
                ctx.guild.id,
            )
    ...
    @skull.command(
        name="channel",
        description="configure the skullboard channel",
        brief="manage guild",
        usage="[channel]",
    )
    @Permissions.has_permission(manage_guild=True)
    async def skull_channel(self, ctx: EvictContext, *, channel: TextChannel):
        ...
        if check is None:
            await self.bot.db.execute(
                "INSERT INTO skullboard (guild_id, channel_id) VALUES ($1, $2)",
                ctx.guild.id,
                channel.id,
            )
        else:
    ...
    async def skull_remove(self, ctx: EvictContext):
        ...
        if check is None:
            return await ctx.warning("skullboard is not **enabled**")
        await self.bot.db.execute(
            "DELETE FROM skullboard WHERE guild_id = $1", ctx.guild.id
        )
        await self.bot.db.execute(
            "DELETE FROM skullboardmes WHERE guild_id = $1", ctx.guild.id
        )
        await ctx.success("Disabled skullboard **succesfully**")

    @skull.command(
        name="stats",
        description="check skullboard stats",
    ...
    @skull.command(
        name="emoji",
        description="configure the skullboard emoji",
        brief="manage guild",
        usage="[emoji]",
    )
    async def skull_emoji(self, ctx: EvictContext, emoji: Union[PartialEmoji, str]):
        ...
        if check is None:
            await self.bot.db.execute(
                "INSERT INTO skullboard (guild_id, emoji_id, emoji_text) VALUES ($1,$2,$3)",
                ctx.guild.id,
                emoji_id,
                str(emoji),
            )
            ...
            await self.bot.db.execute(
                "UPDATE skullboard SET emoji_id = $1 WHERE guild_id = $2",
                emoji_id,
                ctx.guild.id,
            )
            await self.bot.db.execute(
                "UPDATE skullboard SET emoji_text = $1 WHERE guild_id = $2",
                str(emoji),
                ctx.guild.id,
            )
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        try:
            check = await self.bot.db.fetchrow(
                "SELECT * FROM skullboard WHERE guild_id = {}".format(payload.guild_id)
            )
            if check:
                if payload.emoji.is_unicode_emoji():
                    if ord(str(payload.emoji)) != int(check["emoji_id"]):
                        return
                elif payload.emoji.is_custom_emoji():
                    if payload.emoji.id != int(check["emoji_id"]):
                        return
                message = await payload.member.guild.get_channel(
                    payload.channel_id
                ).fetch_message(payload.message_id)
                chec = await self.bot.db.fetchrow(
                    f"SELECT * FROM skullboardmes WHERE guild_id = {payload.guild_id} AND channel_message_id = {payload.channel_id} AND message_id = {payload.message_id}"
                )
                if chec is not None:
                    messa = await payload.member.guild.get_channel(
                        int(chec["channel_skullboard_id"])
                    ).fetch_message(int(chec["message_skullboard_id"]))
                    try:
                        return await messa.edit(
                            content=f"{payload.emoji} **#{sum(r.count for r in message.reactions if str(r.emoji) == str(payload.emoji))}** <#{payload.channel_id}>"
                        )
                    except:
                        pass
                elif chec is None:
                    ...
                    if message.attachments:
                        if (
                            ".mp4" in message.attachments[0].url
                            or ".mov" in message.attachments[0].url
                        ):
                            url = message.attachments[0].url
                            bytes_io = await self.bot.ext.getbyte(url)
                            file = discord.File(
                                fp=bytes_io, filename=message.attachments[0].filename
                            )
                        else:
                    ...
                    async def idk():
                        if isinstance(file, discord.File):
                            mes = await channel.send(
                                content=f"{payload.emoji} **#{reaction.count}** <#{payload.channel_id}>",
                                file=file,
                                embed=embed,
                                view=view,
                            )
                        ...
                        await self.bot.db.execute(
                            "INSERT INTO skullboardmes VALUES ($1,$2,$3,$4,$5)",
                            payload.guild_id,
                            channel.id,
                            payload.channel_id,
                            mes.id,
                        ...
                        if isinstance(reaction.emoji, str):
                            try:
                                if ord(str(reaction.emoji)) == int(check["emoji_id"]):
                                    if reaction.count == int(check["count"]):
                                        await idk()
                            except:
                                pass
                        else:
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        try:
            ...
            if check is not None:
                ...
                if chec is not None:
                    messa = await self.bot.get_channel(
                        int(chec["channel_skullboard_id"])
                    ).fetch_message(int(chec["message_skullboard_id"]))
                    for reaction in message.reactions:
                        if isinstance(reaction.emoji, str):
                            if ord(str(reaction.emoji)) == int(check["emoji_id"]):
                                try:
                                    await messa.edit(
                                        content=f"{reaction.emoji} **#{reaction.count}** <#{payload.channel_id}>"
                                    )
                                except:
                                    pass
                        else:
