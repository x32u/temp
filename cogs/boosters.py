...
class boosters(commands.Cog):
    ...
    @boosterrole.command(
        name="setup", description="set the boosterrole module", brief="manage guild"
    )
    ...
    async def unset(self, ctx: EvictContext):
        ...
        async def yes_callback(interaction: discord.Interaction):
            ...
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{Emojis.approve} {ctx.author.mention}: I have **cleared** the booster role module.",
                ),
                view=None,
            )
        ...
        async def no_callback(interaction: discord.Interaction):
            ...
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    color=Colors.color,
                    description="I did **not** clear the booster role module.",
                ),
                view=None,
            )
    ...
    @boosterrole.command(
        description="set a base role for boosterrole module", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    async def base(self, ctx: EvictContext, *, role: discord.Role = None):
        ...
        if role is None:
            if check is None:
                return await ctx.warning("The booster role base is **not** set.")
        ...
        if role.position >= ctx.guild.get_member(self.bot.user.id).top_role.position:
            return await ctx.warning(
                "You **need** to be above the role you want to use as a base."
            )
    ...
    async def create(self, ctx: EvictContext, name: str = None):
        ...
        if not ctx.author in ctx.guild.premium_subscribers:
            return await ctx.warning("You are **not** a booster.")
        che = await self.bot.db.fetchrow(
            "SELECT * FROM booster_module WHERE guild_id = {}".format(ctx.guild.id)
        )

        if che is None:
            return await ctx.warning("The booster role module is **not** configured.")
    ...
    async def edit(self, ctx: EvictContext):
        ...
        if che is None:
            return await ctx.warning("Booster role module is not configured")
