...
class TicketTopic(discord.ui.Modal, title="Add a ticket topic"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        return await interaction.client.ext.success(
            interaction, f"Added new ticket topic **{self.name.value}**", ephemeral=True
        )
...
class CreateTicket(discord.ui.View):
    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if check is None:
            return await interaction.client.ext.warning(
                interaction, "Ticket module was disabled", ephemeral=True
            )
        ...
        if len(results) == 0:
            text = await interaction.guild.create_text_channel(
                name="ticket-{}".format(interaction.user.name),
                reason="opened ticket",
                category=interaction.client.get_channel(check["category"]) or None,
            )
            ...
            embed.set_footer(
                text="evict.cc", icon_url=interaction.client.user.display_avatar.url
            )
            await interaction.client.db.execute(
            ...
            supportRoles = await interaction.client.db.fetch(
                "SELECT * FROM ticket_support WHERE guild_id = $1", interaction.guild.id
            )
        ...
        async def select_callback(inte: discord.Interaction):
            check = await inte.client.db.fetchrow(
                "SELECT * FROM tickets WHERE guild_id = $1", interaction.guild.id
            )
            if check is None:
                return await interaction.response.send_message(
            ...
            text = await interaction.guild.create_text_channel(
                name="ticket-{}".format(interaction.user.name),
                reason="opened ticket",
                category=interaction.client.get_channel(check[3]) or None,
            )
            overwrites = discord.PermissionOverwrite()
            overwrites.send_messages = True
            overwrites.view_channel = True
            ...
            await inte.client.db.execute(
                "INSERT INTO opened_tickets VALUES ($1,$2,$3)",
                interaction.guild.id,
                text.id,
                interaction.user.id,
            )
            await inte.response.edit_message(
                embed=discord.Embed(
                    color=(
                        int(check["color"])
                        if check["color"] is not None
...
class DeleteTicket(discord.ui.View):
    ...
    async def delete(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def close_callback(inte: discord.Interaction):
            check = await inte.client.db.fetchrow(
                "SELECT logs FROM tickets WHERE guild_id = {}".format(inte.guild.id)
            )
            if check is not None:
                ...
                try:
                    await inte.guild.get_channel(check["logs"]).send(
                ...
                os.remove(filename)
            await inte.client.db.execute(
                "DELETE FROM opened_tickets WHERE channel_id = $1 AND guild_id = $2",
                inte.channel.id,
                inte.guild.id,
            )
            await inte.response.edit_message(
                content=f"closed by {inte.user.mention}", view=None
            )
        ...
        await interaction.response.send_message(
            "Are you sure you want to close the ticket?", view=view
        )
...
class tickets(commands.Cog):
    ...
    async def remove(self, ctx: commands.Context, *, member: discord.Member):
        ...
        return await ctx.success(
            "I have removed **{}** from the ticket.".format(member)
        )
    ...
    async def topics(self, ctx: commands.Context):
        ...
        if check is None:
            return await ctx.reply("no ticket panel created")
        ...
        async def button2_callback(interaction: discord.Interaction):
            ...
            e = discord.Embed(
                color=Colors.color, description=f"🔍 Select a topic to delete."
            )
            options = []
            for result in results:
                options.append(
                    discord.SelectOption(label=result[1], description=result[2])
                )
            select = discord.ui.Select(options=options, placeholder="Select a topic.")
            ...
            async def select_callback(inter: discord.Interaction):
                ...
                await self.bot.ext.success(
                    inter,
                    f"I have removed **{select.values[0]}** topic.",
                    ephemeral=True,
                )
    ...
    @ticket.command(
        description="configure the ticket message",
        usage="[embed code]",
        brief="administrator",
    )
    @Permissions.has_permission(administrator=True)
    async def message(self, ctx: commands.Context, *, message: str = None):
        ...
        if message is not None:
            if check is None:
                await self.bot.db.execute(
                    "INSERT INTO tickets (guild_id, message) VALUES ($1,$2)",
                    ctx.guild.id,
                    message,
                )
            else:
                await self.bot.db.execute(
                    "UPDATE tickets SET message = $1 WHERE guild_id = $2",
                    message,
                    ctx.guild.id,
                )
            return await ctx.success(
                f"I have set the **ticket message** as\n```{message}```"
            )
        else:
            if check is None:
                return await ctx.warning("There is **no** custom ticket message.")
            await self.bot.db.execute(
                "UPDATE tickets SET message = $1 WHERE guild_id = $2",
                None,
                ctx.guild.id,
    ...
    @ticket.command(
        description="configure the ticket support role (will be pinged)",
        usage="[role]",
        brief="administrator",
    )
    @Permissions.has_permission(administrator=True)
    async def support(self, ctx: commands.Context, role: discord.Role = None):
        if role == None:
            await self.bot.db.execute(
                "DELETE FROM ticket_support WHERE guild_id = $1", ctx.guild.id
            )
            return await ctx.success("I have removed **support roles**.")

        await self.bot.db.execute(
            "INSERT INTO ticket_support (guild_id, role_id) VALUES ($1, $2)",
            ctx.guild.id,
            role.id,
        )
    ...
    @ticket.command(
        description="configure the ticket category",
        usage="[category]",
        brief="administrator",
    )
    @Permissions.has_permission(administrator=True)
    async def category(
        self, ctx: commands.Context, *, channel: discord.CategoryChannel = None
    ):
        check = await self.bot.db.fetchrow(
            "SELECT * FROM tickets WHERE guild_id = $1", ctx.guild.id
        )
        if channel is not None:
            if check is None:
                await self.bot.db.execute(
                    "INSERT INTO tickets (guild_id, category) VALUES ($1,$2)",
                    ctx.guild.id,
                    channel.id,
                )
            else:
                await self.bot.db.execute(
                    "UPDATE tickets SET category = $1 WHERE guild_id = $2",
                    channel.id,
                    ctx.guild.id,
                )
            ...
            if check is None:
                return await ctx.warning("Tickets channel is **not** set.")
            await self.bot.db.execute(
                "UPDATE tickets SET category = $1 WHERE guild_id = $2",
                None,
                ctx.guild.id,
    ...
    @ticket.command(
        description="configure the ticket channel",
        usage="[channel]",
        brief="administrator",
    )
    @Permissions.has_permission(administrator=True)
    async def channel(
        ...
        if channel is not None:
            if check is None:
                await self.bot.db.execute(
                    "INSERT INTO tickets (guild_id, channel_id) VALUES ($1,$2)",
                    ctx.guild.id,
                    channel.id,
                )
            else:
                await self.bot.db.execute(
                    "UPDATE tickets SET channel_id = $1 WHERE guild_id = $2",
                    channel.id,
                    ctx.guild.id,
                )
    ...
    @ticket.command(
        description="configure the ticket logging channel",
        usage="[channel]",
        brief="administrator",
    )
    @Permissions.has_permission(administrator=True)
    async def logs(self, ctx: commands.Context, *, channel: discord.TextChannel = None):
        ...
        if channel is not None:
            if check is None:
                await self.bot.db.execute(
                    "INSERT INTO tickets (guild_id, logs) VALUES ($1,$2)",
                    ctx.guild.id,
                    channel.id,
                )
            else:
            ...
            if check is None:
                return await ctx.warning("Ticket logs are **not** set.")
            await self.bot.db.execute(
    ...
    async def send(self, ctx: commands.Context):
        ...
        if check is None:
            return await ctx.warning("You have **not** sent a ticket panel.")
        if ctx.guild.get_channel(check["channel_id"]) is None:
            return await ctx.warning("I could **not** find that channel.")
        ...
        if check["message"]:
            ...
            try:
                x = await EmbedBuilder.to_object(
                    EmbedBuilder.embed_replacement(ctx.author, check["message"])
                )
        ...
        await self.bot.db.execute(
            "UPDATE tickets SET color = $1 WHERE guild_id = $2",
            Colors.color or message.embeds[0].color.value,
            ctx.guild.id,
        )
    ...
    async def settings(self, ctx: commands.Context):
        ...
        if check is None:
            return await ctx.reply("You have **not** created a ticket panel.")
        embed1 = discord.Embed(
            color=Colors.color,
            title="Ticket Settings",
            description="Settings for **{}**".format(ctx.guild.name),
        )
        embed1.add_field(
            name="Ticket Channel",
            value=(
                ctx.guild.get_channel(check["channel_id"]).mention
                if ctx.guild.get_channel(check["channel_id"]) is not None
                else "N/A"
            ),
        )
        embed1.add_field(
            name="Logs Channel",
            value=(
                ctx.guild.get_channel(check["logs"]).mention
                if ctx.guild.get_channel(check["logs"]) is not None
                else "N/A"
            ),
        )
        embed1.add_field(
            name="Category",
            value=(
                ctx.guild.get_channel(check["category"]).mention
                if ctx.guild.get_channel(check["category"]) is not None
                else "N/A"
            ),
        )
