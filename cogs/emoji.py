...
class emoji(commands.Cog):
    ...
    async def deleteemoji(self, ctx: EvictContext, emoji: discord.Emoji):

        await emoji.delete()
        await ctx.success(f"deleted the emoji {emoji.name}.")

    @commands.command(
        description="add an emoji", usage="[emoji] <name>", brief="manage emojis"
    )
    @Permissions.has_permission(manage_expressions=True)
    async def addemoji(
        self,
        ...
        *,
        ...
        try:
            ...
            return await ctx.error(ctx, f"Unable to add the emoji | {e}")
    ...
    async def addmultiple(
        self, ctx: EvictContext, *emoji: Union[discord.Emoji, discord.PartialEmoji]
    ):

        if len(emoji) == 0:
            return await ctx.warning("Please provide some emojis to add")
        emojis = []
    ...
    @Permissions.has_permission(manage_expressions=True)
    async def sticker_steal(self, ctx: EvictContext):
    ...
    async def sticker_enlarge(self, ctx: EvictContext):
        ...
        if ctx.message.stickers:
            ...
            if len(messages) == 0:
                return await ctx.warning("No sticker found")
    ...
    @sticker.command(
        ...
        description="puts your vanity code in the stickers",
    )
    async def sticker_tag(self, ctx: EvictContext):

        if ctx.guild.vanity_url_code == None:
            return await ctx.warning(
                "You **must** have a vanity set to run this command."
            )
        ...
        embed = discord.Embed(
            color=Colors.color,
            description=f"{ctx.author.mention}: I have started **tagging** all the servers stickers.",
        )
        ...
        await message.edit(
            embed=discord.Embed(
                color=Colors.color,
                description=f"{Emojis.approve} {ctx.author.mention}: I have **tagged** all the guilds stickers.",
            )
        )

    @sticker.command(
        name="list", description="returns a list of server's stickers", aliases=["l"]
    )
                ...
                if s.guild_id == ctx.guild.id:
                    ...
                    async def button1_callback(interaction: discord.Interaction):
                        ...
                        return await interaction.response.edit_message(
                            embed=discord.Embed(
                                color=Colors.color,
                                description=f"{Emojis.approve} {interaction.user.mention}: Deleted sticker",
                            ),
                            view=None,
                        )
    ...
    @Permissions.has_permission(manage_expressions=True)
    async def stealsticker(self, ctx: EvictContext):
        ...
        if ctx.message.stickers:
            try:
                ...
                return await ctx.error(ctx, f"Unable to add this sticker - {error}")
            ...
            async for message in ctx.channel.history(limit=10):
                ...
                if message.stickers:
                    ...
                    e = discord.Embed(
                        color=Colors.color, title=message.stickers[0].name
                    ).set_author(
                    ...
                    async def button1_callback(interaction: discord.Interaction):
                        ...
                        try:
                            ...
                            embed = discord.Embed(
                                color=Colors.color, title="sticker added"
                            )
                            embed.set_thumbnail(url=url)
                            embed.add_field(
                                name="values",
                            ...
                            embed = discord.Embed(
                                color=Colors.color,
                                description=f"{Emojis.deny} {ctx.author.mention}: unable to add this sticker",
                            )
                            return await interaction.response.edit_message(
                                embed=embed, view=None
                            )
    ...
    async def emojilist(self, ctx: EvictContext):

        if len(ctx.guild.emojis) == 0:
            return await ctx.warning("There are **no** emojis in this server.")

        emoji_list = [
            f"{emoji} - [**{emoji.name}**]({emoji.url}) (``{emoji.id}``)"
    ...
    async def enlarge(self, ctx: EvictContext, emoj: Union[discord.PartialEmoji, str]):
    ...
    async def emojiinfo(
        ...
        embed = discord.Embed(
            color=Colors.color, title=emoji.name, timestamp=emoji.created_at
    ...
    async def steal(self, ctx: EvictContext, *names: str):
    ...
    @commands.command(
        name="fromimage",
        description="add an emoji from an image",
        brief="manage expressions",
    )
    @commands.has_permissions(manage_expressions=True)
    async def _add_from_image(self, ctx: EvictContext, name: str = None):
