...
class edit(commands.Cog):
    ...
    @Permissions.has_permission(manage_guild=True)
    @gedit.command(
        name="banner", description="change the server banner", usage="[image/link]"
    )
    async def ge_banner(self, ctx: EvictContext, url: str = None):
        ...
        try:
            ...
            await ctx.success(
                f"I have **updated** the server banner to [**image**]({url})."
            )

    @Mod.is_mod_configured()
    @Permissions.has_permission(manage_guild=True)
    @gedit.command(
        name="icon", description="change the server icon", usage="[image/link]"
    )
    async def ge_icon(self, ctx: EvictContext, url: str = None):
        ...
        try:
            ...
            await ctx.success(
                f"I have **updated** the server icon to [**image**]({url})."
            )

    @Mod.is_mod_configured()
    @Permissions.has_permission(manage_guild=True)
    @gedit.command(
        name="splash", description="change the server splash", usage="[image/link]"
    )
    async def ge_splash(self, ctx: EvictContext, url: str = None):
        ...
        try:
            ...
            await ctx.success(
                f"I have **updated** the server invite splash to [**image**]({url})."
            )
    ...
    async def ge_description(
        self, ctx: EvictContext, *, description: typing.Optional[str] = None
    ) -> None:
        async with ctx.typing():
            await ctx.guild.edit(
    ...
    async def invites(self, ctx: EvictContext, *, invites_disabled: bool) -> None:
        async with ctx.typing():
            ...
            await ctx.success(
                f"I have **updated** the server invites setting to {invites_disabled}."
            )

    @Mod.is_mod_configured()
    @Permissions.has_permission(manage_guild=True)
    @gedit.command(
        name="discovery", description="toggle discovery", usage="[true / false]"
    )
    async def ge_discovery(self, ctx: EvictContext, *, discoverable: bool) -> None:
        async with ctx.typing():
            ...
            await ctx.success(
                f"I have **updated** the server discovery settings to {discoverable}."
            )
    ...
    async def ce_nsfw(
        ...
        *,
        ...
        await ctx.success(
            f"I have **updated** {channel.mention}'s nsfw settings to {nsfw}."
        )
    ...
    async def ce_category(
        ...
        await ctx.success(
            f"I have **updated** the category for {channel.mention} to {category.mention}."
        )
    ...
    @Permissions.has_permission(manage_channels=True)
    async def ce_delete(
        self, ctx: EvictContext, channel: typing.Optional[discord.TextChannel] = None
    ):
        if channel is None:
            channel = ctx.channel
    ...
    async def ce_sync(
        ...
        await ctx.success(
            f"I have **updated** the sync permissions for {channel.mention} to `{sync_permissions}`."
        )
    ...
    @Mod.is_mod_configured()
    @cedit.command(
        name="topic", description="change the channel topic", usage="[channel] [topic]"
    )
    @Permissions.has_permission(manage_channels=True)
    async def ce_topic(
        self,
    ...
    async def re_hoist(self, ctx: EvictContext, role: GoodRole, state: str):
        if not state.lower() in ["true", "false"]:
            return await ctx.error(f"**{state}** can be only **true** or **false**")
    ...
    @Mod.is_mod_configured()
    @redit.command(
        name="color", description="change a role's color", usage="[role] [color]"
    )
    @Permissions.has_permission(manage_roles=True)
    async def re_color(self, ctx: EvictContext, role: GoodRole, *, color: str):
        try:
            ...
            return await ctx.reply(
                embed=discord.Embed(
                    color=role.color,
                    description=f"{Emojis.approve} {ctx.author.mention}: Changed role's color",
                )
            )
        except:
            return await ctx.error("Unable to change the role's color")
    ...
    @Permissions.has_permission(manage_threads=True)
    async def te_name(
        self, ctx: EvictContext, thread: typing.Optional[discord.Thread], name: str
    ) -> None:
        if thread is None:
    ...
    @tedit.command(
        name="bump",
        description="keep a thread from auto archiving",
        brief="manage channels",
        usage="[thread]",
    )
    @Permissions.has_permission(manage_threads=True)
    async def te_bump(
        self, ctx: EvictContext, thread: typing.Optional[discord.Thread]
    ) -> None:
        if thread is None:
            if isinstance(ctx.channel, discord.Thread):
                thread = ctx.channel
        check = await self.bot.db.fetchrow(
            "SELECT * FROM threadbumper WHERE thread_id = $1", thread.id
        )
        if check:
            return await ctx.warning(
                f"{thread.mention} is **already** in threadbumoer."
            )
        await ctx.success(f"{thread.mention} has been added to thread bumper.")
        await self.bot.db.execute(
            "INSERT INTO threadbumper VALUES ($1, $2)", ctx.guild.id, thread.id
        )

    @Mod.is_mod_configured()
    @tedit.command(
        name="unbump",
        description="remove a thread from auto archiving",
        brief="manage channels",
        usage="[thread]",
    )
    @Permissions.has_permission(manage_threads=True)
    async def te_unbump(
        ...
        await self.bot.db.execute(
            f"DELETE FROM threadbumper WHERE guild_id = {ctx.guild.id} AND thread_id = {thread.id}"
        )
    ...
    @Permissions.has_permission(manage_channels=True)
    async def ve_name(
        self, ctx: EvictContext, channel: discord.VoiceChannel, name: str
    ):
        await channel.edit(
            name=name,
    ...
    @Permissions.has_permission(manage_channels=True)
    async def ve_nsfw(
        self, ctx: EvictContext, channel: discord.VoiceChannel, nsfw: bool = None
    ):
        if nsfw is None:
    ...
    async def ve_userlimit(
        self, ctx: EvictContext, channel: discord.VoiceChannel, user_limit: int
    ):
