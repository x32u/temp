...
class joindm(commands.Cog):
    ...
    @commands.group(
        invoke_without_command=True, description="manage dm's on member join"
    )
    ...
    @joindm.command(
        name="add",
        description="add a message that will be sent to any new member that will join the server",
        brief="manage guild",
        usage="[message | --embed embed name]",
    )
    ...
    @joindm.command(
        name="remove", description="remove the joindm message", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    async def joindm_remove(self, ctx: EvictContext):
        ...
        if not check:
            return await ctx.warning("Joindm not enabled")
        await self.bot.db.execute(
    ...
    @joindm.command(
        name="test", description="test the joindm message", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
