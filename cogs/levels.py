...
class leveling(commands.Cog):
    ...
    async def rank(self, ctx: EvictContext, member: discord.Member = None):
        ...
        if check is None:
            return await ctx.error("Levels **aren't** enabled in this server.")
        ...
        if che is None:
            return await ctx.reply(
                embed=discord.Embed(color=Colors.color, title=f"{member.name}'s rank")
    ...
    @level.group(
        invoke_without_command=True, description="manage the rewards for each level"
    )
    async def rewards(self, ctx: EvictContext):
        await ctx.create_pages()

    @rewards.command(
        description="add a level reward", usage="[level] [role]", brief="manage guild"
    )
    ...
    async def add(self, ctx: EvictContext, level: int, *, role: discord.Role):
        ...
        await ctx.success(
            f"I have **added** {role.mention} for level **{level}** reward."
        )

    @rewards.command(
        description="remove a level reward", usage="[level]", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    ...
    @rewards.command(
        name="reset", description="reset all level rewards", brief="administrator"
    )
    ...
    async def rewards_reset(self, ctx: EvictContext):
        ...
        if len(results) == 0:
            return await ctx.error("There are **no** role rewards in this server.")
        await self.bot.db.execute(
    ...
    async def list(self, ctx: EvictContext):
        ...
        for table in results:
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(color=Colors.color, description=auto).set_author(
    ...
    async def level_reset(self, ctx: EvictContext, *, member: discord.Member = None):
        ...
        if check is None:
            return await ctx.warning("Levels are not configured.")
        if not member:
            await self.bot.db.execute(
    ...
    async def leaderboard(self, ctx: EvictContext):
        ...
        if len(results) == 0:
            return await ctx.error("nobody is on the **level leaderboard**")
    ...
    @level.command(
        description="enable leveling system, or disable it", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    ...
    async def levelup(self, ctx: EvictContext, destination: str):
        if not destination in ["dms", "channel", "off"]:
            return await ctx.warning("You passed an **invalid** destination.")
        check = await self.bot.db.execute(
            "SELECT * FROM levelsetup WHERE guild_id = {}".format(ctx.guild.id)
        )
        if check is None:
            return await ctx.reply(
                "The leveling system is **not** enabled,", mention_author=False
            )
        await self.bot.db.execute(
        ...
        return await ctx.success(
            f"I have **updated** the level up message destination: **{destination}**."
        )
    ...
    async def channel(self, ctx: EvictContext, *, channel: discord.TextChannel):
        ...
        if check is None:
            return await ctx.warning("The leveling system is **not** enabled.")
        if channel is None:
            await self.bot.db.execute(
            ...
            return await ctx.success(
                "I have **removed** the channel for level up messages."
            )
            ...
            await ctx.success(
                f"I have set the channel for level up messages to {channel.mention}."
            )
