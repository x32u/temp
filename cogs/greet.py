...
class greet(commands.Cog):
    ...
    @boost.command(
        name="variables", description="return the variables for the boost message"
    )
    ...
    async def boost_config(self, ctx: EvictContext):
        ...
        if not res:
            return await ctx.warning("Boost is not **configured**")
        channel = (
            f'#{ctx.guild.get_channel(res["channel_id"]).name}'
            if ctx.guild.get_channel(res["channel_id"])
            else "none"
        )
        e = res["mes"] or "none"
        embed = discord.Embed(
            color=Colors.color, title=f"channel {channel}", description=f"```{e}```"
        )
    ...
    async def boost_message(self, ctx: EvictContext, *, code: str):
        ...
        if check:
            await self.bot.db.execute(
                "UPDATE boost SET mes = $1 WHERE guild_id = $2", code, ctx.guild.id
            )
        else:
            await self.bot.db.execute(
                "INSERT INTO boost VALUES ($1,$2,$3)", ctx.guild.id, 0, code
            )
        return await ctx.success(f"Configured boost message as `{code}`")

    @boost.command(
        name="channel",
        description="configure the boost channel",
        brief="manage guild",
        usage="[channel]",
    )
    @Permissions.has_permission(manage_guild=True)
    async def boost_channel(
        ...
        if channel is None:
            ...
            if not check:
                return await ctx.warning("Boost **channel** is not configured")
            await self.bot.db.execute(
                "UPDATE boost SET channel_id = $1 WHERE guild_id = $2",
                None,
                ctx.guild.id,
            )
            ...
            await ctx.success(
                "Configured boost **channel** to {}".format(channel.mention)
            )

    @boost.command(
        name="delete", description="delete the boost module", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    async def boost_delete(self, ctx: EvictContext):
        ...
        if not check:
            return await ctx.warning("Boost module is not configured")
        ...
        if res:
            ...
            try:
                x = await EmbedBuilder.to_object(
                    EmbedBuilder.embed_replacement(ctx.author, res["mes"])
                )
            ...
            await ctx.success(
                "Sent the **boost** message to {}".format(channel.mention)
            )
    ...
    @leave.command(
        name="variables", description="return the variables for the leave message"
    )
    ...
    async def leave_config(self, ctx: EvictContext):
        ...
        if not res:
            return await ctx.warning("Leave is not **configured**")
        channel = (
            f'#{ctx.guild.get_channel(res["channel_id"]).name}'
            if ctx.guild.get_channel(res["channel_id"])
            else "none"
    ...
    async def leave_message(self, ctx: EvictContext, *, code: str):
        ...
        if check:
            await self.bot.db.execute(
                "UPDATE leave SET mes = $1 WHERE guild_id = $2", code, ctx.guild.id
            )
        else:
            await self.bot.db.execute(
                "INSERT INTO leave VALUES ($1,$2,$3)", ctx.guild.id, 0, code
            )
    ...
    async def leave_channel(
        ...
        if channel is None:
            check = await self.bot.db.fetchrow(
                "SELECT channel_id FROM leave WHERE guild_id = $1", ctx.guild.id
            )
            if not check:
                return await ctx.warning("Leave **channel** is not configured")
            await self.bot.db.execute(
            ...
            await ctx.success(
                "Configured leave **channel** to {}".format(channel.mention)
            )

    @leave.command(
        name="delete", description="delete the leave module", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    async def leave_delete(self, ctx: EvictContext):
        ...
        if not check:
            return await ctx.warning("Leave module is not configured")
    ...
    async def leave_test(self, ctx: EvictContext):
        ...
        if res:
            ...
            await ctx.success(
                "Sent the **leave** message to {}".format(channel.mention)
            )
    ...
    @welcome.command(
        name="variables", description="return the variables for the welcome message"
    )
    ...
    async def welcome_config(self, ctx: EvictContext):
        ...
        if not res:
            return await ctx.warning("Welcome is not **configured**")
        channel = (
            f'#{ctx.guild.get_channel(res["channel_id"]).name}'
            if ctx.guild.get_channel(res["channel_id"])
            else "none"
    ...
    async def welcome_message(self, ctx: EvictContext, *, code: str):
        ...
        if check:
            await self.bot.db.execute(
                "UPDATE welcome SET mes = $1 WHERE guild_id = $2", code, ctx.guild.id
            )
        else:
            await self.bot.db.execute(
                "INSERT INTO welcome VALUES ($1,$2,$3)", ctx.guild.id, 0, code
            )
    ...
    async def welcome_channel(
        ...
        if channel is None:
            ...
            if not check:
                return await ctx.warning("Welcome **channel** is not configured")
            await self.bot.db.execute(
            ...
            await ctx.success(
                "Configured welcome **channel** to {}".format(channel.mention)
            )

    @welcome.command(
        name="delete", description="delete the welcome module", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    async def welcome_delete(self, ctx: EvictContext):
        ...
        if not check:
            return await ctx.warning("Welcome module is not configured")
        await self.bot.db.execute(
    ...
    @welcome.command(
        name="test", description="test welcome module", brief="manage guild"
    )
    @Permissions.has_permission(manage_guild=True)
    async def welcome_test(self, ctx: EvictContext):
        ...
        if res:
            ...
            await ctx.success(
                "Sent the **welcome** message to {}".format(channel.mention)
            )
