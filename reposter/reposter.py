...
class Reposter:
    ...
    async def repost_twitter(
        self, bot: commands.Bot, message: discord.Message, url: str
    ):
        ...
        if len(tweet.media.videos) >= 1:
            file = discord.File(
                fp=await self.read_file_from_url(tweet.media.videos[0].variants[1].url),
                filename="evict-twitter.mp4",
            )
        ...
        view.add_item(
            discord.ui.Button(
                ...
                label="View On 𝕏",
                url=tweet.url,
                emoji="<:twitter:1219531899106496574>",
            )
        )
        embed = discord.Embed(
            color=0x1DA1F2,
            description=f"[**{tweet_content if len(tweet_content) >= 1 else 'No Caption'}**]({tweet.url})",
            timestamp=tweet.date,
        )
        embed.set_author(
            name=f"{tweet.user.displayname} (@{tweet.user.username})",
            icon_url=tweet.user.profileImageUrl,
            url=tweet.user.url,
        )
        embed.set_footer(
            icon_url="https://www.iconpacks.net/icons/2/free-twitter-logo-icon-2429-thumb.png",
            text="❤️ {}  🔁 {} ∙ Requested by: {}".format(
                tweet.likeCount, tweet.retweetCount, message.author
            ),
        )
        ...
        return (
            ...
            if file is not None
            else await message.channel.send(embed=embed, view=view)
        )

    async def repost_youtube(
        self, bot: commands.Bot, message: discord.Message, url: str
    ):
        await message.channel.typing()
        ydl_opts = {
        ...
        channel = yt_dlp.YoutubeDL({"playlist_items": "0"}).extract_info(
            info["uploader_url"]
        )
        ...
        if info["age_limit"] != 0:
            return await message.channel.send(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{message.author.mention}: I can't repost possibly **sensitive** videos.",
                )
            )
        ...
        embed = discord.Embed(
            color=Colors.color,
            description=f"[**{info['title'][:100] + '...' if len(info['title']) > 150 else info['title']}**]({info['webpage_url']})",
        )
        formats = [
            f
        ...
        if len(formats) == 0:
            return print("Could not find any formats")

        file = discord.File(
            fp=await self.read_file_from_url(formats[0]["url"]),
            filename="evict-youtube.mp4",
        )
        embed.set_author(
            name=f"{info['uploader']} ({info['uploader_id']})",
            url=info["uploader_url"],
            icon_url=channel["thumbnails"][-1]["url"],
        )
        embed.set_footer(
            icon_url=message.author.display_avatar.url,
            text=f"Requested by {message.author} ﹒ Views: {views} ﹒Likes: {likes}",
        )
        view = discord.ui.View()
        view.add_item(
            discord.ui.Button(
                ...
                emoji="<:youtube:1219530840070160384>",
            )
        )
        ...
        async with locks[message.guild.id]:
            ...
            async with message.channel.typing():
                ...
                if x["data"].get("images"):
                    ...
                    video = x["data"]["play"]
                    file = File(
                        fp=await session.getbyte(video), filename="evicttiktok.mp4"
                    )
                    embed = Embed(
                        color=Colors.color,
                        description=(
