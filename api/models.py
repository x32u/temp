...
class Worker:
    ...
    def join(self, token: str, invite: str):
        if not self.check_token(token):
            raise HTTPException(status_code=401, detail="Bad token was provided")
    ...
    def fetch_user(self, user_id: int, token: str, guild_id: Optional[int] = None):
        ...
        if r.status_code == 200:
            ...
            if guild_id:
                payload["pronouns"] = (
                    data["guild_member_profile"]["pronouns"]
                    if data["guild_member_profile"]["pronouns"] != ""
                    else data["user_profile"]["pronouns"]
                )
                payload["bio"] = (
                    data["guild_member"]["bio"]
                    if data["guild_member"]["bio"] != ""
                    else data["user"]["bio"]
                )
                ...
                if not av:
                    ...
                    payload["avatar"] = (
                        f"https://cdn.discordapp.com/guilds/{guild_id}/users/{user_id}/avatars/{av}.{'gif' if 'a_' in av else 'png'}?size=1024"
                        if av == data["guild_member"]["avatar"]
                        else f"https://cdn.discordapp.com/avatars/{user_id}/{av}.{'gif' if 'a_' in av else 'png'}?size=1024"
                    )
                ...
                if not banner:
                    ...
                    payload["banner"] = (
                        f"https://cdn.discordapp.com/guilds/{guild_id}/users/{user_id}/banners/{banner}.{'gif' if 'a_' in banner else 'png'}?size=512"
                        if banner == data["guild_member"]["banner"]
                        else f"https://cdn.discordapp.com/banners/{payload['user_id']}/{banner}.{'gif' if 'a_' in banner else 'png'}?size=512"
                    )
            else:
                ...
                if av := data["user"].get("avatar"):
                    payload["avatar"] = (
                        f"https://cdn.discordapp.com/avatars/{user_id}/{av}.{'gif' if 'a_' in av else 'png'}?size=1024"
                    )
                else:
                    payload["avatar"] = None
                ...
                if banner := data["user"].get("banner"):
                    payload["banner"] = (
                        f"https://cdn.discordapp.com/banners/{user_id}/{banner}.{'gif' if 'a_' in banner else 'png'}"
                    )
                else:
                    payload["banner"] = None
