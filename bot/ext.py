    ...
    async def success(
        ...
        if isinstance(ctx, commands.Context):
            return await ctx.reply(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{Emojis.approve} {ctx.author.mention}: {message}",
                )
            )
        else:
            return await ctx.response.send_message(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{Emojis.approve} {ctx.user.mention}: {message}",
                ),
                ephemeral=ephemeral,
            )
        ...
        if isinstance(ctx, commands.Context):
            return await ctx.reply(
                embed=discord.Embed(
                    ...
                    description=f"{Emojis.deny} {ctx.author.mention}: {message}",
                )
            )
        else:
            return await ctx.response.send_message(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{Emojis.deny} {ctx.user.mention}: {message}",
                ),
                ephemeral=ephemeral,
            )
    ...
    async def warning(
        ...
        if isinstance(ctx, commands.Context):
            return await ctx.reply(
                embed=discord.Embed(
                    color=Colors.error_color,
                    description=f"{Emojis.warn} {ctx.author.mention}: {message}",
                )
            )
        else:
            return await ctx.response.send_message(
                embed=discord.Embed(
                    color=Colors.color,
                    description=f"{Emojis.warn} {ctx.user.mention}: {message}",
                ),
                ephemeral=ephemeral,
            )
...
class GoToModal(discord.ui.Modal, title="change the page"):
    page = discord.ui.TextInput(
        label="page", placeholder="change the page", max_length=3
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embed=self.embeds[int(self.page.value) - 1]
        )
    ...
    async def on_error(
        ...
        await interaction.client.ext.warning(
            interaction, "I am unable to change the page", ephemeral=True
        )
