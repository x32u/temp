...
class Permissions:
    ...
    def staff():
        ...
        async def staff(ctx: commands.Context):
            if ctx.author.id in STAFF:
                ...
                raise commands.CommandInvokeError(
                    "You **need** to have staff permissions to use staff commands."
                )
    ...
    def booster():
        ...
        async def booster(ctx: commands.Context):
            ...
            if che is None:
                raise commands.CommandInvokeError(
                    "the booster role module is **not** configured."
                )
            ...
            if check is None:
                raise commands.CommandInvokeError("You don't have a booster role.")
    ...
    def donor():
        ...
        async def donor(ctx: commands.Context):
            ...
            if check is None:
                raise commands.CommandInvokeError(
                    "You need to be a donor to run this command."
                )
...
class Whitelist:
    ...
    async def whitelist_things(
        ...
        check = await ctx.bot.db.fetchrow(
            ...
            (
                ...
                or isinstance(target, discord.User)
                else "channel"
            ),
        )
        if check:
    ...
    async def unwhitelist_things(
        ...
        if not check:
            return await ctx.warning(
                f"{f'**{target}**' if isinstance(target, discord.Member) else target.mention} is **not** whitelisted from **{module}**"
            )
        await ctx.bot.db.execute(
        ...
        return await ctx.success(
            f"{f'**{target}**' if isinstance(target, discord.Member) else target.mention} is now unwhitelisted from **{module}**"
        )
