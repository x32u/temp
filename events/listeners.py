...
class listeners(commands.Cog):
    ...
    async def on_booster_lost(self, before: discord.Member, after: discord.Member):
        if (
            ...
            await self.bot.db.execute(
                "INSERT INTO boosterslost VALUES ($1,$2,$3)",
                before.guild.id,
                before.id,
    ...
    async def on_booster_get(self, before: discord.Member, after: discord.Member):
        if (
            ...
            await self.bot.db.execute(
                "DELETE FROM boosterslost WHERE guild_id = $1 AND user_id = $2",
                before.guild.id,
                before.id,
            )
    ...
    async def jail(self, member: discord.Member):
        check = await self.bot.db.fetchrow(
            "SELECT * FROM jail WHERE guild_id = {} AND user_id = {}".format(
                member.guild.id, member.id
            )
        )
        if check:
            chec = await self.bot.db.fetchrow(
                "SELECT * FROM mod WHERE guild_id = {}".format(member.guild.id)
            )
            if chec:
                try:
                    await member.add_roles(
                        member.guild.get_role(int(chec["role_id"])),
                        reason="jailed before leaving",
                    )
    ...
    async def autorole(self, member: discord.Member):
        ...
        if check:
            try:
                await member.add_roles(
                    member.guild.get_role(int(check["role_id"])), reason="autorole"
                )
            except discord.Forbidden:
                return
    ...
    async def on_user_update(self, before, after):
        ...
        data = await self.bot.db.fetchrow(
            "SELECT user FROM nodata WHERE user_id = $1 AND state = $2",
            before.id,
            "false",
        )
        if data:
            return
        await self.bot.db.execute(
            "INSERT INTO oldusernames VALUES ($1,$2,$3,$4)",
            before.name,
    ...
    async def hardban_check(self, guild: discord.Guild, user: discord.User):
        ...
        if check is not None:
            try:
                await guild.ban(
                    user,
                    reason=f"hardbanned by {await self.bot.fetch_user(check['author'])}",
                )
            except discord.Forbidden:
                return
  ...
  async def booster_unboosted(self, before: discord.Member, after: discord.Member): 
    ...
    async def owner_ban_check(self, guild: discord.Guild, user: discord.User):
        if user.id in self.bot.owner_ids:
            try:
                await guild.unban(
                    user,
                    reason="User cannot be banned. Kick evict to ban this user.",
                )
            except discord.Forbidden:
                await guild.leave()
    ...
    async def autokick_check(self, member: discord.User):
        ...
        if check is not None:
            try:
                await guild.kick(
                    member,
                    reason=f"autokicked by {await self.bot.fetch_user(check['author'])}",
                )
            except discord.Forbidden:
                return
    ...
    async def private_check(self, member: discord.User):
        guild = member.guild
        check = await self.bot.db.fetchrow(
            "SELECT * FROM private WHERE guild_id = {}".format(guild.id)
        )
        check1 = await self.bot.db.fetchrow(
            "SELECT * FROM private WHERE private_users = $1", member.id
        )
        if not check:
            return
        if not check1:
            try:
                await member.kick(
                    reason=f"private is enabled, whitelist this user to let them join."
                )
            except discord.Forbidden:
                return
    ...
    async def forcenick_check(self, before: discord.Member):
        check = await self.bot.db.fetchrow(
            "SELECT nickname FROM forcenick WHERE user_id = {} AND guild_id = {}".format(
    ...
    async def forcenick1_check(self, before: discord.Member, after: discord.Member):
        if str(before.nick) != str(after.nick):
            ...
            if check:
                try:
                    await before.edit(
                        nick=check["nickname"], reason="forcenick enabled"
                    )
                except:
                    pass
            ...
            if res:
                ...
                try:
                    x = await EmbedBuilder.to_object(
                        EmbedBuilder.embed_replacement(message.author, res["mes"])
                    )
        ...
        if (
            ...
            if not before.guild.system_channel:
                ...
                if res:
                    ...
                    try:
                        x = await EmbedBuilder.to_object(
                            EmbedBuilder.embed_replacement(before, res["mes"])
                        )
                        await channel.send(content=x[0], embed=x[1], view=x[2])
                    except:
                        await channel.send(
    ...
    async def leave_message(self, member: discord.Member):
        ...
        if res:
            ...
            try:
                x = await EmbedBuilder.to_object(
                    EmbedBuilder.embed_replacement(member, res["mes"])
                )
                await channel.send(content=x[0], embed=x[1], view=x[2])
            except:
                await channel.send(EmbedBuilder.embed_replacement(member, res["mes"]))
                ...
                if res["destination"] == "channel" or res["destination"] is None:
                    ...
                    try:
                        await message.author.send(
                            f"you leveled up to level `{lvl_start + 1}` in **{message.guild.name}**"
                        )
                    except:
                        pass
                await self.bot.db.execute(
