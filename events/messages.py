...
class Messages(commands.Cog):
    ...
    async def webhook(self, channel) -> discord.Webhook:
        for webhook in await channel.webhooks():
            if webhook.user == self.bot.user:
                return webhook
        try:
            await channel.create_webhook(name="evict")
        except:
            pass
    ...
    async def subscriber_join(self, member: discord.Member):
        if member.guild.id == 1208651928507129887:
            ...
            if check:
                await member.add_roles(member.guild.get_role(1209127936414842990))
                ...
                try:
                    ...
                    await message.channel.send(
                        embed=discord.Embed(
                            color=Colors.color,
                            title="AutoMod",
                            description=f"{Emojis.warn} {message.author.mention}: You have been muted for **1 minute** for sending discord invites in this channel",
                        )
                    )
                except KeyError:
                    pass
                ...
                try:
                    ...
                    await message.channel.send(
                        embed=discord.Embed(
                            color=Colors.color,
                            title="AutoMod",
                            description=f"{Emojis.warn} {message.author.mention}: You have been muted for **5 minutes** for sending discord invites in this channel",
                        )
                    )
                except KeyError:
                    pass
