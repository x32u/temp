            ...
            try:
                x = await EmbedBuilder.to_object(
                    EmbedBuilder.embed_replacement(
                        member,
                        InvokeClass.invoke_replacement(
    ...
    async def invoke_cmds(
        ...
        if res:
            ...
            if embed == "none":
                ...
                return await ctx.success(
                    f"Updated your custom **{ctx.command.name}** message to {'the embed' if '--embed' in embed else ''}\n```{embed}```"
                )
        else:
            await ctx.bot.db.execute(
            ...
            return await ctx.success(
                f"Added your custom **{ctx.command.name}** message to {'the embed' if '--embed' in embed else ''}\n```{embed}```"
            )
