...
class EmbedField:
    ...
    def to_variable(cls, field: EmbedField):
        return (
            f'field: {field.name} && {field.value} {"inline" if field.inline else ""}'
        )
    ...
    def from_dict(cls, data: dict):
        return cls(
            ...
            style=(
                ...
                if data.get("style") == "red"
                else (
                    discord.ButtonStyle.green
                    if data.get("style") == "green"
                    else (
                        discord.ButtonStyle.gray
                        if data.get("style") == "gray"
                        else (
                            discord.ButtonStyle.blurple
                            if data.get("style") == "blue"
...
class Embed:
    ...
    def replace(code: str, member: discord.Member, target: discord.Member):
        ...
        code = code.replace(
            "{server.owner.avatar}", member.guild.owner.display_avatar.url
        )
        if member.guild.banner is not None:
            code = code.replace("{server.banner}", member.guild.banner.url)
        code = code.replace("{server.region}", "I dont know")
        code = code.replace("{server.member_count}", str(member.guild.member_count))
        code = code.replace(
            "{server.boost_count}", str(member.guild.premium_subscription_count)
        )
        code = code.replace("{server.boost_tier}", str(member.guild.premium_tier))
        code = code.replace(
            "{server.created_at}",
            discord.utils.format_dt(member.guild.created_at, style="R"),
        )
        code = code.replace(
            "{server.vanity}", "/" + (member.guild.vanity_url_code or "none")
        )
        ...
        code = code.replace(
            "{guild.owner.avatar}", member.guild.owner.display_avatar.url
        )
        if member.guild.banner is not None:
            code = code.replace("{guild.banner}", member.guild.banner.url)
        code = code.replace("{guild.region}", "I dont know")
        code = code.replace("{guild.member_count}", str(member.guild.member_count))
        code = code.replace(
            "{guild.boost_count}", str(member.guild.premium_subscription_count)
        )
        ...
        code = code.replace(
            "{guild.vanity}", "/" + (member.guild.vanity_url_code or "none")
        )
    ...
    @classmethod
    def from_dict(
        cls, data: dict, member: discord.Member, target: discord.Member = None
    ):
        if target is None:
            target = member
        return cls(
            ...
            author=Embed.replace(
                EmbedAuthor.from_dict(data.get("author"), member, target)
            ),
            footer=EmbedFooter.from_dict(
                Embed.replace(data.get("footer"), member, target)
            ),
            fields=[
                EmbedField.from_dict(field)
                for field in Embed.replace(data.get("fields", []), member, target)
            ],
            buttons=[
                EmbedButton.from_dict(button)
                for button in Embed.replace(data.get("buttons", []), member, target)
            ],
        )

    @classmethod
    def from_variable(
        cls, variables: str, member: discord.Member, target: discord.Member = None
    ):
        if target is None:
            target = member
        (
        ...
        if len(variables) == 1 and "{embed}" not in variables:
            return cls(
                content=variables[0],
                title=title,
                color=color,
        ...
        variables = [
            variable[1:-1] if "{" in variable else variable[:-1]
            for variable in variables
        ]
        for variable in variables:
            if variable.startswith("content:"):
                content = variable[len("content:") :]
            if variable.startswith("title:"):
                title = variable[len("title:") :]
            if variable.startswith("description:"):
                description = variable[len("description:") :]
            if variable.startswith("color:"):
                color = int(variable[len("color:") :].replace("#", ""), 16)
            if variable.startswith("image:"):
                image = variable[len("image:") :]
            if variable.startswith("thumbnail:"):
                thumbnail = variable[len("thumbnail:") :]
            if variable.startswith("author:"):
                author = EmbedAuthor.from_variable(variable[len("author:") :])
            if variable.startswith("footer:"):
                footer = EmbedFooter.from_variable(variable[len("footer:") :])
            if variable.startswith("field:"):
                fields.append(EmbedField.from_variable(variable[len("field:") :]))
            if variable.startswith("button:"):
                buttons.append(EmbedButton.from_variable(variable[len("button:") :]))
            if variable.startswith("timestamp"):
    ...
    def to_variables(message: discord.Message) -> str:
        ...
        if message.content:
            variables.append(f"content:{message.content}")
        if embed is None:
            return "{" + "}{".join(variables) + "}"
        if embed.title:
            variables.append(f"title:{embed.title}")
        if embed.description:
            variables.append(f"description:{embed.description}")
        if embed.color:
            variables.append(f"color:{embed.color}")
        if embed.image:
            variables.append(f"image:{embed.image}")
        if embed.thumbnail:
            variables.append(f"thumbnail:{embed.thumbnail.url}")
        if embed.author:
